...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userSubcrption.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-subcrptions.update", [$userSubcrption->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="selected_days">{{ trans('cruds.userSubcrption.fields.selected_days') }}</label>
                <input class="form-control {{ $errors->has('selected_days') ? 'is-invalid' : '' }}" type="text" name="selected_days" id="selected_days" value="{{ old('selected_days', $userSubcrption->selected_days) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('start_date') ? 'is-invalid' : '' }}" type="text" name="start_date" id="start_date" value="{{ old('start_date', $userSubcrption->start_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('end_date') ? 'is-invalid' : '' }}" type="text" name="end_date" id="end_date" value="{{ old('end_date', $userSubcrption->end_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $userSubcrption->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subcrption_plans') ? 'is-invalid' : '' }}" name="subcrption_plans_id" id="subcrption_plans_id" required>
                    @foreach($subcrption_plans as $id => $entry)
                        <option value="{{ $id }}" {{ (old('subcrption_plans_id') ? old('subcrption_plans_id') : $userSubcrption->subcrption_plans->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('duration') ? 'is-invalid' : '' }}" name="duration_id" id="duration_id" required>
                    @foreach($durations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('duration_id') ? old('duration_id') : $userSubcrption->duration->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $userSubcrption->price) }}" step="1">
                @if($errors->has('price'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment') ? 'is-invalid' : '' }}" name="payment" id="payment">
                    ...
                    @foreach(App\Models\UserSubcrption::PAYMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment', $userSubcrption->payment) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\UserSubcrption::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $userSubcrption->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
