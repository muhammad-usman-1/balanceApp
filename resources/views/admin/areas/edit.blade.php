...
<div class="card idx-card">
    <div class="card-header">
        <h3><i class="fas fa-pen mr-2" style="color:#b45309;"></i> Edit Area</h3>
        ...
        <form method="POST" action="{{ route('admin.areas.update', $area->id) }}">
                ...
                <div class="mf-field">
                    ...
                    <div class="mf-suffix">
                        <input class="mf-input {{ $errors->has('delivery_charges') ? 'is-err' : '' }}"
                               ...
                               step="0.01" min="0" value="{{ old('delivery_charges', $area->delivery_charges) }}" required>
                        <span>KWD</span>
                    </div>
                    ...
                    <div class="toggle-wrap" style="margin-top:6px;">
                        <label class="toggle">
                            <input type="checkbox" name="status" id="status" value="active"
                                   {{ old('status', $area->status) === 'active' ? 'checked' : '' }}>
                            <span class="toggle-slider"></span>
                        </label>
                        <span class="toggle-label" id="toggle-text">
                            {{ old('status', $area->status) === 'active' ? 'Active' : 'Inactive' }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div style="margin-top: 8px; padding-top: 20px; border-top: 1px solid #f3f4f6;
                ...
                <button type="submit" class="idx-btn ib-edit" style="padding: 9px 22px; font-size: .85rem;">
                    <i class="fas fa-check"></i> Update Area
                </button>
