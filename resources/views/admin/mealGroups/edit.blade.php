...
.mf-input:focus { border-color: #b45309; box-shadow: 0 0 0 3px rgba(180,83,9,.08); }
...
.mg-picker-count {
    font-size: .78rem; font-weight: 700; color: #b45309; white-space: nowrap; margin-left: auto;
}
...
.mg-picker-item.is-checked { background: #fffbeb; }
.mg-picker-item input[type=checkbox] { width: 16px; height: 16px; accent-color: #b45309; cursor: pointer; flex-shrink: 0; }
...
@php
    $assignedIds = old('meal_ids', $mealGroup->meals->pluck('id')->toArray());
@endphp

<div class="card idx-card">
    <div class="card-header">
        <h3><i class="fas fa-pen mr-2" style="color:#b45309;"></i> Edit Meal Group</h3>
    ...
    <div class="card-body" style="padding:24px !important;">
        <form method="POST" action="{{ route('admin.meal-groups.update', $mealGroup) }}">
            @csrf
            @method('PUT')
            ...
            <div style="display:grid; grid-template-columns: 1fr 200px; gap:14px; margin-bottom:20px;">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="mf-input @error('name') is-invalid @enderror"
                           value="{{ old('name', $mealGroup->name) }}" required>
                ...
                <div>
                    ...
                    <input type="number" name="weekly_limit" id="weekly_limit" min="1" max="7"
                           ...
                           value="{{ old('weekly_limit', $mealGroup->weekly_limit) }}" required>
            ...
            <div class="mg-picker">
                ...
                <div class="mg-picker-list" id="mgPickerList">
                    @forelse($meals as $meal)
                        ...
                        <label class="mg-picker-item" data-search="{{ strtolower($meal->title.' '.($meal->category->name ?? '')) }}">
                            <input type="checkbox" name="meal_ids[]" value="{{ $meal->id }}"
                                   {{ in_array($meal->id, $assignedIds) ? 'checked' : '' }}>
                            @if($thumb)
                    ...
                    @empty
                        <div class="mg-picker-empty">No available meals.</div>
                    @endforelse
                </div>
            </div>
            <small class="form-text text-muted">Meals already in this group, plus any meal not currently in another group, are shown.</small>
            ...
            <div style="margin-top:24px; padding-top:20px; border-top:1px solid #f3f4f6; display:flex; gap:10px;">
                <button type="submit" class="idx-btn ib-edit" style="padding:9px 22px; font-size:.85rem;">
                    <i class="fas fa-check"></i> Update Group
                </button>
