...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.subscriptionMeal.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.subscription-meals.update", [$subscriptionMeal->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subscription_days') ? 'is-invalid' : '' }}" name="subscription_days_id" id="subscription_days_id" required>
                    @foreach($subscription_days as $id => $entry)
                        <option value="{{ $id }}" {{ (old('subscription_days_id') ? old('subscription_days_id') : $subscriptionMeal->subscription_days->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('meal') ? 'is-invalid' : '' }}" name="meal_id" id="meal_id" required>
                    @foreach($meals as $id => $entry)
                        <option value="{{ $id }}" {{ (old('meal_id') ? old('meal_id') : $subscriptionMeal->meal->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('meal'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    <option value="is meal" {{ (old('type') ? old('type') : $subscriptionMeal->type ?? '') == 'is meal' ? 'selected' : '' }}>Is Meal</option>
                    <option value="is snack" {{ (old('type') ? old('type') : $subscriptionMeal->type ?? '') == 'is snack' ? 'selected' : '' }}>Is Snack</option>
                </select>
                @if($errors->has('type'))
