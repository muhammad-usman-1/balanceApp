...
<div class="cp-wrap">
    <div class="card idx-card">
        <div class="card-header">
            <h3><i class="fas fa-ticket-alt mr-2" style="color:#db2777;"></i> Edit Coupon</h3>
        </div>
            ...
            <form action="{{ route('admin.coupons.update', $coupon->id) }}" method="POST">
                ...
                <div class="cp-row">
                    <div class="cp-group">
                        ...
                        <input type="text" name="coupon_code" id="coupon_code"
                               ...
                               placeholder="e.g. SUMMER20" value="{{ old('coupon_code', $coupon->coupon_code) }}" required>
                ...
                <div class="cp-group">
                    ...
                    <div class="cp-type-row">
                        <label class="cp-type-opt">
                            <input type="radio" name="type" value="fixed" {{ old('type', $coupon->type) === 'fixed' ? 'checked' : '' }} required>
                            Fixed Amount
                        </label>
                        <label class="cp-type-opt">
                            <input type="radio" name="type" value="percentage" {{ old('type', $coupon->type) === 'percentage' ? 'checked' : '' }}>
                            Percentage
                        </label>
                    </div>
                ...
                <div class="cp-group">
                    ...
                    <div class="cp-value-group">
                        ...
                        <span class="cp-value-suffix" id="value_suffix">{{ $coupon->type === 'percentage' ? '%' : 'KWD' }}</span>
                    </div>
                    @error('value')
                        <div class="cp-error">{{ $message }}</div>
                    @else
                        <div class="cp-help" id="value_help">{{ $coupon->type === 'percentage' ? 'Enter percentage value (0-100)' : 'Enter the fixed discount amount' }}</div>
                    @enderror
                </div>
                ...
                <div class="cp-group">
                    ...
                    <div class="cp-status-row">
                        <label class="cp-status-opt">
                            <input type="radio" name="status" value="active" {{ old('status', $coupon->status) === 'active' ? 'checked' : '' }} required>
                            Active
                        </label>
                        <label class="cp-status-opt">
                            <input type="radio" name="status" value="inactive" {{ old('status', $coupon->status) === 'inactive' ? 'checked' : '' }}>
                            Inactive
                        </label>
                    </div>
                ...
                <div class="cp-actions">
                    <button type="submit" class="cp-btn-submit"><i class="fas fa-check"></i> Update Coupon</button>
