...
.mf-input:focus { border-color: #b45309; box-shadow: 0 0 0 3px rgba(180,83,9,.1); }
...
.br-picker-count { font-size: .78rem; font-weight: 700; color: #b45309; white-space: nowrap; margin-left: auto; }
...
.br-picker-item.is-checked { background: #fffbeb; }
.br-picker-item input[type=checkbox] { width: 16px; height: 16px; accent-color: #b45309; cursor: pointer; flex-shrink: 0; }
...
@php
    $assignedAreaIds = old('areas', $branch->areas->pluck('id')->toArray());
@endphp

<div class="card idx-card">
    <div class="card-header">
        <h3><i class="fas fa-pen mr-2" style="color:#b45309;"></i> Edit Branch</h3>
        ...
        <form method="POST" action="{{ route('admin.branches.update', $branch->id) }}">
            ...
            <div class="mf-field">
                ...
                <div class="toggle-wrap" style="margin-top:4px;">
                    ...
                    <span class="toggle-label" id="toggle-text">
                        {{ old('status', $branch->status) === 'active' ? 'Active' : 'Inactive' }}
                    </span>
                </div>
            </div>
            ...
            <div class="br-picker">
                ...
                <div class="br-picker-list" id="brPickerList">
                    @forelse($areas as $area)
                        <label class="br-picker-item" data-search="{{ strtolower($area->name) }}">
                            <input type="checkbox" name="areas[]" value="{{ $area->id }}"
                                   {{ in_array($area->id, $assignedAreaIds) ? 'checked' : '' }}>
                            <div>
                    ...
                    @empty
                        <div class="br-picker-empty">No available areas. All active areas are already assigned to other branches.</div>
                    @endforelse
                </div>
            </div>
            @if($errors->has('areas'))
