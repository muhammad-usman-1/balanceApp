...
<head>
    ...
    <title>All Delivery Notes – {{ $date->format('d M Y') }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logo-text { font-size: 32px; font-weight: 300; letter-spacing: 8px; text-transform: uppercase; color: #222; }
...
<div class="no-print" style="padding:12px; background:#f8f9fa; border-bottom:1px solid #dee2e6; text-align:right; position:sticky; top:0; z-index:10;">
    <strong style="margin-right:12px; font-size:13px;">{{ $deliveryOrders->count() }} delivery notes – {{ $date->format('l, d M Y') }}</strong>
...
<div class="page">
    ...
    <table class="items-table">
        ...
        <tbody>
            @if($meals->count())
            <tr><td colspan="2" class="section-label">Meals</td></tr>
            @foreach($meals as $sm)
            <tr>
                <td>
            ...
            @if($snacks->count())
            <tr><td colspan="2" class="section-label">Snacks</td></tr>
